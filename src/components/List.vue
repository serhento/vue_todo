<template>
    <ul class="todo__list">
        <li
                :class="[{active: index === SELECTED_TASK}, 'active--'+item.color.name]"
        ...
        >
            ...
            <svg @click.stop="deleteList(index, item.id)" v-if="!item.img" width="11" height="11" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        methods:{
            ...
            deleteList(index, id){
                if (window.confirm('Вы действительно хотите удалить список?')){
                    if (this.SELECTED_TASK === index){
                        this.DELETE_LIST_FROM_LISTS(id);
                        this.SET_ACTIVE_TASK(0);
                    }else {
                        this.DELETE_LIST_FROM_LISTS(id);
                    }
                }
